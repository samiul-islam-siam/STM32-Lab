...
void SystemClock_Config(void)
{
    ...
    /* PLL: 16/8=2 MHz x 180 = 360 MHz VCO / 2 = 180 MHz */
    RCC->PLLCFGR  = 0;
...
uint8_t USART2_TryReceiveByte(uint8_t *outByte)
{
    if (USART2->SR & USART_SR_RXNE) {
        *outByte = (uint8_t)USART2->DR;
        return 1U;
    }
    return 0U;
}
...
/*
 * TIM2 for 1 ms tick interrupt
 * fTIM2_CLK = 90 MHz (APB1 timer clock, x2 since APB1 prescaler != 1)
 * PSC = 89  -> tick = 1 us
 * ARR = 999 -> overflow every 1000 us = 1 ms
 */
void TIM2_Init(void)
{
